...
from metamodeler.config import bootstrap, diagnose, format_diagnose_report
from metamodeler.config.bootstrap import find_repo_root
from metamodeler.config.diagnose import diagnose_to_json
from metamodeler.config.platform import (
    detect_env,
    detect_os,
...
def test_format_diagnose_report_renders_lines():
    ...
    assert "Metamodeler environment diagnostic" in rendered
...
def test_find_repo_root_walks_up_from_subdir(tmp_path):
    ...
    assert 'name = "metamodeler"' in text
...
def test_bootstrap_is_idempotent():
    ...
    root2 = bootstrap()
    assert root1 == root2
    assert (root1 / "src" / "metamodeler").is_dir()
