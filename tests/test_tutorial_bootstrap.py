...
from metamodeler.tutorial import bootstrap, find_repo_root, run_cli


def test_bootstrap_returns_repo_root_with_marker():
    root = bootstrap()
    assert (root / "pyproject.toml").is_file()
    assert (root / "src" / "metamodeler").is_dir()


def test_run_cli_executes_help_without_shell(tmp_path):
    # Calling `mm --help` via the helper should succeed and print usage text.
    result = run_cli("--version", capture=True)
    assert result.returncode == 0
    assert "metamodeler" in result.stdout.lower()
