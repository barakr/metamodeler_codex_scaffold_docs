"""System diagnostic for `mm doctor`.
...
from metamodeler.config._import_helpers import probe_package
from metamodeler.config.bootstrap import find_repo_root
from metamodeler.config.platform import snapshot
...
def _build_recommendations(
    ...
    if repo_root is None:
        recs.append(
            "Could not locate the metamodeler repo root. "
            "Run `mm doctor` from inside the project directory."
        )
    ...
    if not backends["pymc"]["installed"] or not backends["arviz"]["installed"]:
        ...
        recs.append("Install PyMC backend (pip): `pip install 'metamodeler[pymc]'`")
    if not backends["torch"]["installed"] or not backends["sbi"]["installed"]:
        ...
        recs.append("Install SBI backend (pip): `pip install 'metamodeler[sbi]'`")
...
def format_diagnose_report(report: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("Metamodeler environment diagnostic")
    lines.append("=" * 40)
