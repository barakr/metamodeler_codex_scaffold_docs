...
from metamodeler.config._import_helpers import (
    require_pymc as _require_pymc,
)
from metamodeler.config._import_helpers import (
    require_sbi as _require_sbi,
)
from metamodeler.config._import_helpers import (
...
class LinearGaussianModel:
    """Single-output linear+Gaussian fallback. D=1 only."""
    ...
    def summary(self, inputs: dict[str, np.ndarray]) -> dict:
        mean = self._mean(inputs)
        return {
            "mean": {self.output_name: mean.tolist()},
            "sigma": {self.output_name: self.sigma},
            "n": int(len(mean)),
        }
...
class PymcPosteriorLinearModel:
    ...
    Shapes (D = number of outputs, F = number of input features, S = number of posterior draws):
    ...
    def _predictive_mu(self, inputs: dict[str, np.ndarray]) -> np.ndarray:
        ...
        # weights: (S, F, D) → mu_i = x @ weights[s] + bias[s]
        mu = np.einsum("nf,sfd->nsd", x, self.posterior_weights) + self.posterior_bias[None, :, :]
        return mu

    def sample(self, inputs: dict[str, np.ndarray], n: int, seed: int) -> np.ndarray:
        """Posterior-predictive samples of shape (N, n, D)."""
    ...
    def log_prob(self, inputs: dict[str, np.ndarray], outputs: dict[str, np.ndarray]) -> np.ndarray:
        """Per-row log probability of the observed outputs under the posterior predictive.

        Returns shape (N,). For multi-output, the per-row log-prob is the joint log-prob
        across all output dimensions (sum over D for diagonal; full MvNormal for full).
        Averaged over posterior draws via logsumexp.
        """
        mu = self._predictive_mu(inputs)  # (N, S, D)
        n_rows, n_draws, d = mu.shape
        y = np.column_stack(
...
class SbiNPEPosteriorModel:
    ...
    posterior; calling ``.sample((n,), x=obs)`` returns shape ``(n, D)``.
    For ``output_correlation == "diagonal"``, ``posteriors`` holds D 1-D posteriors,
    each producing shape ``(n, 1)``.
    """
    ...
    def sample(self, inputs: dict[str, np.ndarray], n: int, seed: int) -> np.ndarray:
        ...
        for idx, row in enumerate(x_norm):
            ...
            if self.output_correlation == "full":
                ...
                out[idx] = self._denormalize(stacked)
        return out
    ...
    def log_prob(self, inputs: dict[str, np.ndarray], outputs: dict[str, np.ndarray]) -> np.ndarray:
        ...
        for idx, row in enumerate(x_norm):
            ...
            if self.output_correlation == "full":
                ...
                value = posterior.log_prob(theta.unsqueeze(0), x=obs)
                scalar = float(np.asarray(value.detach().cpu().numpy(), dtype=float).reshape(-1)[0])
                logp.append(scalar + affine_correction)
            else:
                running = affine_correction  # log y_scale already covers all dims
                for j, posterior in enumerate(self.posteriors):
    ...
    def summary(self, inputs: dict[str, np.ndarray]) -> dict:
        draws = self.sample(inputs=inputs, n=self.summary_samples, seed=0)  # (N, n, D)
        return {
            "mean": {
